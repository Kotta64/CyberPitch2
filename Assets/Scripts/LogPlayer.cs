...
public class LogPlayer : MonoBehaviour
{
    ...
    private bool setLog()
    {
        ...
        string[] path = StandaloneFileBrowser.OpenFilePanel("Open rcg File", "", "rcg", false);
        if (path.Length == 0) return false;

        string text = ReadText(path[0]);
    ...
    IEnumerator sendLog()
    {
        while (true) {
            if(frame < LogList.Count) {
                ...
                GameManager.instance.AddLog(GameManager.instance.GetLog(-1));
                yield return new WaitForSeconds(speed);
            }
